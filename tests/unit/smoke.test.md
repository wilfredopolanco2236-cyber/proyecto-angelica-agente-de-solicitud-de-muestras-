...
6. Input: `estado sm20260425-sab-0009` -> intent `CHECK_STATUS` and normalized request number.
7. Duplicate `messageId` -> idempotent no-op response.
8. Unauthorized user executing `allowlist.add` -> blocked by policy guard.

Automatable fixtures: `tests/unit/scenarios.json`.
